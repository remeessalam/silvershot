      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-7 max-w-5xl mx-auto mt-5">
        {portfolioList.map(({ img, id, title }) => (
          <div
            ...
            data-aos="fade-up"
          >
            <div className="absolute top-0 w-full h-full bg-gradient-to-b from-transparent to-darkbg z-0"></div>
            <img
              loading="lazy"
              src={img}
              width="200"
              height="200"
              className={`-z-10 h-full w-full object-cover grayscale group-hover:grayscale-0 brightness-90`}
              alt={title}
            />
            <h3 className="text-xl absolute p-3 bottom-0 left-0 w-full bg-darkbg text-center text-white">
              {title}
            </h3>
          </div>
        ))}
