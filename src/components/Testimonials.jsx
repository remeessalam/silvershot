        ...
        <div className="flex items-center gap-4 w-full">
          ...
          <div ref={sliderRef} className="keen-slider" data-aos="fade-up">
            {testimonials.map((item) => (
              <div
                key={item.id}
                className="keen-slider__slide text-center justify-between text-white p-5 bg-darkbg rounded-xl flex flex-col gap-4"
              >
                <p className="italic desc">{item.desc}</p>
                <div className="flex flex-col gap-2 mt-2">
                  <div className="flex flex-col items-center gap-1 h-full">
                    <div>
                      <FaUserCircle className="h-full w-[2rem] rounded-full" />
                    </div>
                    {/* <img
                      loading="lazy"
                      src={item.img}
